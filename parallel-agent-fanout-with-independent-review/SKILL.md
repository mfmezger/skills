...
---
## When to Use
This skill IS "graph engineering" applied to code work: model the job as a graph — nodes that do work (worker/reviewer subagents), edges that route between them (spawn, GO/REWORK, rebase, merge), and shared state flowing along the edges (the base branch). Trigger phrases: "graph engineering", "graph mode", "run the graph", "run graph engineering on X", "fan out and review", "parallel PRs with independent review". When invoked by any of those, load and follow this skill.

Use when a review/audit produced several independent findings and you want them implemented concurrently with real verification — typically 3+ fixes across one or two repos, each small enough for its own PR (each fix is a node; its PR is the node's output). Also use whenever agent-authored code will be merged: the independent-reviewer node is the highest-value edge and is worth running even for a single fix.

It is graph engineering, NOT knowledge graphs / GraphRAG — this is about modelling execution (which agent runs next, what state it gets), not modelling data. Do NOT reach for it for one small change (a single loop is cheaper and correct), for exploratory work with no clear finish line per node, or when the findings are actually one entangled change — splitting coupled work into parallel PRs manufactures the semantic conflicts this skill then has to clean up. The default answer to "should this be a graph?" is usually no; escalate from a single loop to a graph only when the work forces distinct specialties, fan-out/join, or a dedicated verifier node.
